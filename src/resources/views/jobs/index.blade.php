                    ...
                    <table class="border-collapse table-auto w-full text-sm">
                        ...
                        @foreach ($jobs as $job)
                            <tr>
                                ...
                                <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{{ $job->salary }}</td>
                                <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{{ $job->type }}</td>
                                <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{{ $job->created_at }}</td>
                                <td class="flex inline-flex justify-center border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                    ...
                                    @endif
                                    @if(Auth::user()->type === 'admin')
                                    <form method="post" action="{{ route('jobs.destroy', $job->id) }}" class="mb-0 h-9 mx-6 inline">
                                        @csrf
                                        @method('delete')
                                        <button class="border border-red-500 hover:bg-red-500 hover:text-white px-4 py-4 rounded-md">
                                            Excluir
                                        </button>
                                    </form>
                                    @endif
